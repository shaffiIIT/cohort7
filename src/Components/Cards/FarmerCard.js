...
const FarmerCard = ({id, farmerObject}) => {
  ...
  console.log("FrameCard authState.address  #########: "+authState.address + "  id adress  "+id);
  useEffect(() => {
    //if(farmerObject){
    if(!contractState.farmerContract){
      (async() => {
        console.log("FrameCard getting farmer name######: "+(await fetchFarmer(authState.address, contractState.farmerContract, id)).name);
      ...
      (async() => {
        console.log("getting farmer name: "+(await fetchFarmer(authState.address, contractState.farmerContract, id)).name);
        setFarmer(await fetchFarmer(authState.address, contractState.farmerContract, id));
      })();
    }
  }, [farmerObject])
